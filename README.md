...
Parsed downloads are cached under `data/raw/` (git-ignored). The app prefers a committed snapshot under `data/snapshot/` (`prices.csv.gz`, `boundaries.geojson.gz`, `cpi.csv.gz`, `demographics.csv.gz`, `demographics_national.json.gz`, `municipality_prices.csv.gz`, `municipalities.geojson.gz`, `manifest.json`) so cold starts do not hit the live APIs. The municipality snapshot files are optional: if they are missing, the map behaves as before (postal-only) and the UI says so.
