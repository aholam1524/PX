...
Interactive map and analysis for **Finnish housing prices** by postal-code area. The Streamlit app has **Map**, **Affordability**, **Compare**, **Relationships**, and **My home** tabs. On **Map**, a **MapLibre choropleth** colours postal-code boundaries by your chosen metric (price per square metre, nominal or inflation-adjusted 1- or 5-year change, sales in the last four quarters, **market activity** (trailing sales per 1,000 Paavo inhabitants—a rough index, not housing-stock turnover), a rough price-to-income ratio, **payment share of median household income** (loan payment vs Paavo household income for a typical flat), or **Fits my budget**). Value layers use a **light-grey to black** scale (**darker = higher**); areas with no published value stay **unfilled** (outline only) so they are not confused with low values. By default **Fill gaps with municipality values** is on: where a postal-code area has no published price, a **lighter municipality fill** underneath shows the yearly municipality average (postal detail stays on top wherever it exists). Municipality values are **annual** figures from Statistics Finland’s municipality table; postal-code values stay **quarterly**. Turn that checkbox off to return to the postal-only map. The **price per square metre** layer uses a fixed **0 to 4,000 EUR/m²** colour scale (areas above 4,000 use the darkest colour); other value layers use the 2nd–98th percentile of the visible values (postal and municipality combined) so a few extreme areas do not flatten the rest. Turn on **Use the full value range** in the sidebar to stretch the price layer (or any layer) from minimum to maximum instead. The map caption reports how many areas have their own postal price, how many are coloured from municipality averages, and how many remain grey. Use the sidebar to pick quarter and building type (the default quarter favours complete coverage over the newest provisional release); the map opens centred on mainland Finland with **00100 Helsinki keskusta** selected in the detail panel on first load; hover for details, click the map or use the area search box to change the selected area (type part of a postal code, area name, or municipality)—clicking a municipality-coloured gap opens a short **municipality figure** card instead of the full postal detail panel. The **Affordability** tab answers “Where can I afford to live?”: a summary counts how many areas fit your sidebar budget, and a sortable table lists each area with typical price (EUR/m² × size), monthly payment, headroom, **median household income** (Paavo), **payment share of that income** with comfortable / stretched / hard classes, and the same **fits / slightly over / over** labels as the **Fits my budget** map layer. **What if rates rise?** stress-tests the selected area (+0 to +3 percentage points on the rate, optional income drop). Payment-share classes (≤30 % comfortable, 30–40 % stretched, above 40 % hard) are **rules of thumb**, not official lending limits. Select a row to choose that area for the Map tab and the rest of the app; loan maths and the disclaimer live under **How the numbers are calculated**. Sidebar inputs (apartment size, monthly budget, interest rate, loan term, down payment) are the single source for the tab and the map layer—the calculator does **not** fetch or predict rates and is an illustration only, not advice. The budget map layer colours areas green, amber, or red from those same rules (grey where neither postal nor municipality price exists). The **area detail panel** (beside the map) shows key figures, rank and reliability, a quarterly **trend chart** with municipality and national comparison lines (gaps where data is missing), sales volume from 2020, flagged unusual quarter-on-quarter moves, and a CSV download; you can **add the selected area to comparison**. On **Compare**, pick up to four areas (type part of a postal code, area name, or municipality in the area list) to see a side-by-side metrics table, one multi-series price chart (optional index to 100 at the start), and **similar areas** for a chosen comparison area. The **Relationships** tab scatter-plots price per m² against Paavo income and demographic shares (with correlation and an exclusion count). On **My home**, enter where and when you bought and your purchase price to see how the area’s average price per m² has moved since then and a rough illustration of what your home might be worth now (purchase price × area price index). The tab shows nominal and inflation-adjusted change, reliability, an indexed trend chart from your purchase quarter, and plain-language notes when quarters are missing. **This is not a valuation** — it applies average area movement to your price; condition, floor, view, and renovation are not in the data. Nothing you type is stored or sent anywhere. The map can colour areas by a rough **price-to-income** ratio (municipality fill is not shown for that layer or for trailing sales). Unfilled areas have no published value at either level; amber borders and hover notes mark low-reliability estimates.
...
**Yearly municipality prices** (coarser fallback when a postal-code area has no published price) come from Statistics Finland’s table [“Prices per square meter of old dwellings in housing companies and numbers of transactions by municipality, yearly”](https://pxdata.stat.fi/PxWeb/api/v1/en/StatFin/ashi/13mx.px) (`13mx.px`, calendar years). Loader: `housing_analyzer/data/municipalities.py` (`load_municipality_prices()` / `fetch_municipality_prices()`). Transaction counts use the same asset-transfer basis as the postal-code table (`lkm_julk20` from 2020 onward; `lkm_julk19` for earlier years when needed). The municipality list follows the **2015** classification in that table, while committed postal boundaries use the **2022** edition—some municipality codes exist in only one source; `municipality_join_report()` lists them (same idea as the postal-code join). Merged municipality polygons for a future map layer are derived by dissolving postal-code areas on `kuntanro` / `kunta` (`load_municipality_boundaries()` / `build_municipality_boundaries()`).

**Paavo demographics and income** (population, median disposable income per person, share aged 65+, share with higher university degree, household size and tenure, median disposable **household** income, dwellings, main type of activity) come from Statistics Finland’s open database [Postinumeroalueittainen_avoin_tieto](https://pxdata.stat.fi/PxWeb/api/v1/en/Postinumeroalueittainen_avoin_tieto/) (`uusin` tables **12ey.px**, **12f1.px**, **12ez.px**, **12f2.px**, **12f3.px**, **12f4.px**, **12f6.px**, reference year **2024**, postal variable `postinumeroalue_4_20260101`). Household income uses **`tr_mtu`** (median disposable monetary income of households) from **12f3.px**. Loader: `housing_analyzer/data/demographics.py` (`load_demographics()` / `fetch_demographics()`). Derived fields include rented and owner-occupied household shares, **unemployment rate** as unemployed ÷ (employed + unemployed) (labour-force rate, not share of all inhabitants), student and pensioner shares of inhabitants, and the share of dwellings in blocks of flats. National totals for comparison use the **`SSS`** (whole country) row, stored separately from postal-code rows. Paavo uses a newer postal-code edition than the 2022 price table and boundaries; codes that exist in only one dataset stay missing on join, not imputed (`join_demographics_to_postal_codes` reports the mismatched codes for callers that need them).

**Average rents per square metre** (for the rent-vs-buy tab and gross rental yield layer) come from Statistics Finland’s table [“Rent index (2025=100) and average rents per square meter, quarterly”](https://pxdata.stat.fi/PxWeb/api/v1/en/StatFin/asvu/15fa.px) (`15fa.px`). Rents are published only for **regions, Greater Helsinki (`pks`), the rest of the country (`msu`), and selected large cities**—not by postal code—and the quarterly series starts in **2025** (a short history at first). Loader: `housing_analyzer/data/rents.py` (`load_rents()` / `fetch_rents()`). For each postal-code area, the rent row is chosen from the area’s municipality number (`kuntanro` on the boundary): **city code** when that municipality has its own row in the rent table, otherwise the **maakunta** as `MKxx`, otherwise **`pks`** for Helsinki, Espoo, Vantaa and Kauniainen when no region match exists, otherwise **`msu`**. Municipality-to-maakunta links come from Statistics Finland’s [open classifications API](https://data.stat.fi/api/classifications/v2/correspondenceTables/kunta_1_20250101%23maakunta_1_20250101/maps) (CC BY 4.0) and are stored as `municipality_region.csv` in the snapshot (`rent_area_for_municipality()`).

Parsed downloads are cached under `data/raw/` (git-ignored). The app prefers a committed snapshot under `data/snapshot/` (`prices.csv.gz`, `boundaries.geojson.gz`, `cpi.csv.gz`, `demographics.csv.gz`, `demographics_national.json.gz`, `municipality_prices.csv.gz`, `municipalities.geojson.gz`, `rents.csv.gz`, `municipality_region.csv`, `manifest.json`) so cold starts do not hit the live APIs. The municipality and rent snapshot files are optional until refreshed: if they are missing, the map behaves as before (postal-only) and the UI says so.
...
- **Relationships tab** (`housing_analyzer/analysis/relationships.py`): Scatter plots of price per m² vs income, demographic shares, unemployment rate (labour force), and rented-household share for areas with OK price reliability and complete Paavo fields; shows sample size, Pearson *r*, and a short note that correlation is not causation.
...
- **Rent vs buy** (`housing_analyzer/rent_vs_buy.py`, **Rent vs buy** tab): Compares postal-code (or municipality fallback) purchase prices with **non-subsidised** average rents for the mapped room class. Shows gross rental yield, price-to-rent ratio, and a monthly rent vs loan-plus-charges table using sidebar loan assumptions and an editable maintenance-charge assumption (default 4 EUR/m²). Rents are **region- or city-level** only (from 2025 onward)—rough comparisons, not street-level advice.
- **Gross rental yield map layer**: Colours each postal-code area that has a price using yield = rent × 12 / price, with rent taken from the area’s rent region (`rent_area_for_municipality()`). No municipality price fill; missing price or rent stays unfilled. Hidden when `rents.csv.gz` is not in the snapshot.
